...
const Slide = (props) => (
  <div className={styles.slideContainer}>
    {[...Array(props.slideCount)].map((_, index) => (
      ...
      >
        Slide {index + 1}
        {props.activeSlideIndex === index + 1 && props.slideCount > 3 && (
          <img
            onClick={async () => {
...
const MobileAddStory = (props) => {
    ...
    if (id) {
      const getPost = async () => {
        const response = await fetch(
          `${conf.backendUrl}/api/post/postDetails/${id}`,
        );
        if (response.ok) {
  ...
  const handleCategoryChange = (index, value) => {
    ...
    newPostData.slides = newPostData.slides.map((slide) => ({
      ...slide,
      category: value,
    }));
  ...
  const handlePost = async () => {
    ...
    try {
      const response = await fetch(`${conf.backendUrl}/api/post/add`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        props.onStoryChange(postData.slides[0].category);
        navigate("/");
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${conf.backendUrl}/api/post/edit/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `${localStorage.getItem("token")}`,
      ...
      if (response.ok) {
        props.onStoryChange(payload[0].category);
        navigate("/");
  ...
  return (
    <ModalContainer>
      ...
      <div>
        ...
        >
          {isProcessing ? "Loading..." : id ? "Update" : "Post"}
        </button>
      </div>
    </ModalContainer>
  );
